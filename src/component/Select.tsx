...
export const Select = ({ options, label, className = '', onChange, ...props }: SelectProps) => {
    ...
    const value = Reflect.get(state, props.name) ?? '';
    ...
    const onChangeHandler = useCallback((ev: React.ChangeEvent<HTMLSelectElement>) => {
        console.log('Select changed:', state, props.name, ev.currentTarget.value);
        Reflect.set(state, props.name, ev.currentTarget.value);
        onChange?.(ev);
    }, [state, props.name, onChange]);

    return (
        <div className="w-full">
            {labelCmp}
            <select {...props} onChange={onChangeHandler} defaultValue={value} className={`bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 w-full ${className}`}>
